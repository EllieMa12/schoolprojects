...
#   Locations 
# ---------------------------------------------------------------------------- 
# PRJDIR is the folder you received as a zip file
# See the project2.pdf for a description
PRJDIR = '/Users/machenxi/PycharmProjects/FINS 5546/z5249090'
...
Q4 = 'It is something that is traded, like crypto or fiat currency.'
...
Q6 = 'It consists of a base and quote, and is traded on an exchange. Base and quote are both an asset.'
...
Q8 = 'It is a pair listed on an exchange. For example, "btceur" on exchange "kraken" is a market.'
...
def to_json(obj, pth):
    ...
    with open (pth,"w") as fobj:
        json.dump(obj, fobj, indent= 4, sort_keys= True)
    return f'object has been serialized'
...
def get_ohlc_loc(exchange, symbol):
    ...
    """
    # <COMPLETE THIS PART>
    pth = f'{FILESDIR}/{exchange}_{symbol}.json'
    return pth
...
def write_exchanges():
    ...
    return f'Exchange file has been created'
...
def read_exchanges():
    ...
    """
    # <COMPLETE THIS PART>
    dic = from_json(EXCHANGES)['result']
    col_map = {'1. active': 'active',
               '2. name': 'name',
               '3. route': 'route',
               '4. symbol': 'symbol',
    }
...
def write_ohlc(exchange, symbol, filesdir=None):
    ...
    endpoint = f'https://api.cryptowat.ch/markets/{exchange}/{symbol}/ohlc'
    res = requests.get(endpoint)
    pth = get_ohlc_loc(exchange,symbol)
    to_json(res.json(),pth)

    return f'Jason file has been saved'
...
def candle_to_dic(lst):
    ...
    for i,n in enumerate(lst):
        dic[key_map[i]] = n
    dic['timestamp'] = int(dic['timestamp'])
...
def period_to_secs(period):
    ...
    if period in _valid_values:
        tran_secs = secs[_valid_values.index(period)]
...
def read_ohlc(exchange, symbol, period):
    ...
    # <COMPLETE THIS PART>
    data = from_json(pth)['result']
    lst = data[period_to_secs(period)]
    new_lst = [candle_to_dic(list) for list in lst]
...
def mk_ohlc_df(exchange, symbol, period):
    ...
    #data = read_ohlc(exchange = 'kraken', symbol= 'btceur', period= '5m')

    # <COMPLETE THIS PART>
    df = pd.DataFrame(data)
    df.loc[:, 'utc_dt'] = pd.Series(dt.datetime.utcfromtimestamp(time) for time in df.loc[:, 'timestamp'])
...
def get_all_data(valid_exch):
    ...
    """
    # <COMPLETE THIS PART>
    for value in valid_exch:
        write_ohlc(value,SYMBOL)

    return f'All data has been gotten'
